...
- **长任务查询状态** — 通过 `get_task_status` 获取状态和 `lotNo`；完成或停止后再调用 `export_data`
- **监控用量** — 追踪每周提取的记录数，保持在限制范围内

另请参阅：<a href="/zh/mcp/index" target="_blank" rel="noopener noreferrer">MCP 概览</a>、<a href="/zh/mcp/troubleshooting" target="_blank" rel="noopener noreferrer">故障排查</a>
