...
认证方式：**OAuth 2.1**（推荐）或 **API Key**（Header：`x-api-key`）。API Key 创建见 <a href="/zh/mcp/quick-start/api-key" target="_blank" rel="noopener noreferrer">获取 API Key</a>。
...
或使用 OAuth 流程（以 Claude Code 当前版本文档为准）。详见 <a href="/zh/mcp/quick-start/api-key" target="_blank" rel="noopener noreferrer">获取 API Key</a>。
