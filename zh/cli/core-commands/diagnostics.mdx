...
若未登录，请先 <a href="/zh/cli/quick-start/get-api-key-and-log-in" target="_blank" rel="noopener noreferrer">获取 API Key 并登录</a>。
...
| `capabilities --help` 输出不是 JSON | v0.1.33 已修复为显示帮助；需要机器可读能力清单时使用 `bazhuayu capabilities --json` |
| `--template-registration-id` 无法用于新脚本 | v0.1.33 统一使用 `--template-id`；模板公开输出也以 `templateId` 为准 |
| 模板搜索结果数量或跨页结果变化 | v0.1.33 会按 `templateId` 去重后再分页；结果减少或不再重复是预期行为 |
| 安装 `@latest` 卡住 | 使用固定版本：`npm install -g bazhuayu-cli@0.1.33` |
