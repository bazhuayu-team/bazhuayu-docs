...
时间范围与运行列表共用词汇：`created_from` / `created_to` 为 ISO-8601 绝对时间，起点包含、终点不包含，按运行**发起时间**归属。把同样的参数带进 <a href="/zh/datahub/api/reference/runs/list-runs">运行列表</a>，得到的就是任一分组数字背后的运行。`tz_offset` 只影响按日分桶时一次运行落到哪一天，不改变金额，也不改变时间范围本身。
...
* 单次运行的费用构成在运行对象的 `billing` 块里，见<a href="/zh/datahub/api/reference/runs/get-run">查询运行</a>。
