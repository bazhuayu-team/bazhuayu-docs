...
api: GET https://api-datahub.bazhuayu.com/v1/runs
authMethod: bearer
---
...
筛选条件可以组合。`status` 支持逗号分隔多值（OR）；`credential` 是发起凭证的非敏感稳定标识，用来追踪是哪把 Key 在消耗额度；`created_from` / `created_to` 按发起时间，起点包含、终点不包含。这套词汇与<a href="/zh/datahub/api/reference/account/get-billing" target="_blank" rel="noopener noreferrer">账单聚合</a>相同，把账单分组的键原样传进来就能钻取到对应的运行。
