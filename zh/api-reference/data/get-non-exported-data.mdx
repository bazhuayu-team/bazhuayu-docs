---
title: 获取未导出的数据
description: "介绍“获取未导出的数据”接口的功能、参数与调用方法。"
api: GET https://openapi.bazhuayu.com/data/notexported
playground: interactive
authMethod: apiKey
---

> **所需套餐：** 旗舰版、旗舰+、企业版、团队版

## 端点

```
GET https://openapi.bazhuayu.com/data/notexported
```

## 描述

获取任务中尚未标记为已导出的数据。

## 请求头

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `x-api-key` | String | 二选一 | API Key，例如 `op_sk_xxxxxx`；创建方式见 <a href="/zh/mcp/quick-start/api-key" target="_blank" rel="noopener noreferrer">获取 API Key</a> |
| `Authorization` | String | 二选一 | Access Token，格式 `Bearer <access_token>` |

**请求头示例**

```json
{
  "Authorization": "Bearer eyJhbGciOiJSUzI1NiIsImtpZCI6ImdXS3hWc0F4SHhaMDlDcWh6NkNJb3ciLCJ0eXAiOiJhdCtqd3QifQ..ARhbBfzI1xS6yo-Ue1CFHzMaUIfzw4gJlubNBOkUT4SSTlhuS4bVQ4t4UHuYGbE9VX6XgNVgkcDnx6XapqcPohSy7vPv9TIAsjtbrQSA3mj13SnVVAPWwfAVJ-I7g9HDnyPEEOVSvmZUJhiz-irH5L0ojbnt3ck2rQ9tIFuSd7j2eh5BS4ItjNoHY00rh_QPZltk66a9q-fNE33GrxOaXUdHH47WMlsENnh1gybB7QtGtd6bZ982Qa9JljpFnZWSdEf4VfkN4JmhkUTTC908DqS3stT-LD-9tpztFwB54sd0D4CyOBnESkKrUwjxchYYZo4l1O7auiZVGV48YhZYMw"
}
```

## 查询参数

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `taskId` | String | 是 | 任务 Id |
| `size` | Int | 是 | 数据条数，范围：1~1000 |

**请求示例**

```
GET https://openapi.bazhuayu.com/data/notexported?taskId=a3b9bdf7-9aaa-22f6-9ccf-b32790f6df8f&size=4
```

## 响应

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `data` | Object[] | 是 | 响应数据 |
| `data` | Object | 是 | 任务数据 |
| `total` | Int | 是 | 数据总量 |
| `current` | Int | 是 | 该次导出的数据数量 |

**成功示例**

```json
{
  "data": {
    "total": 100000,
    "current": 4,
    "data": [
      {
        "词条": "深圳视界",
        "简介": "领先的信息化解决方案提供商，致力于企业级数据整合，网页数据采集，整理，分析和挖掘。"
      },
      {
        "词条": "八爪鱼",
        "简介": "它彻底改变了大家对爬虫和采集器的认知，让网页数据采集变得前所未有的简单。"
      },
      {
        "词条": "数多多",
        "简介": "致力于打造大数据共享与交易平台。"
      },
      {
        "词条": "微图",
        "简介": "一款简单易用的数据分析及可视化工具，支持自然语言分析及文本挖掘。"
      }
    ]
  },
  "requestId": "0HMD469L0I8Q1:00000001"
}
```

## 错误响应

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `requestId` | String | 是 | 请求Id |
| `error` | Object | 是 | 错误信息，参考错误码 |
| `code` | String | 是 | 错误码 |
| `message` | String | 是 | 错误描述 |

**错误示例**

```json
{
  "error": {
    "code": "ServerError",
    "message": "Internal Server Error"
  },
  "requestId": "0HMD469L0I8R0:00000001"
}
```

<ParamField header="Authorization" type="string" required hidden>
</ParamField>
<ParamField query="taskId" type="string" required hidden>
</ParamField>
<ParamField query="size" type="number" required hidden>
</ParamField>
